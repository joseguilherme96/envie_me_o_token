...
def test_o_rpa_nao_deve_ser_processado_devido_a_ausencia_do_atributo_token(mocker):
    ...
    with app.app_context():
        ...
        mocker.patch.object(
            api,
            "enviar_dados_para_execucao_sp_sadt",
            Mock(
                side_effect=[
                    {
                        "message": "O atributo token não foi enviado !",
...
def test_o_rpa_nao_deve_ser_processado_devido_dados_incompletos_para_execucao_sp_sadt(
    mocker,
):
    ...
    with app.app_context():
        ...
        fake.processar_rpa.side_effect = Exception(
            {"message": "Dados incompletos", "status_code": 400}
        )
        ...
        mocker.patch.object(
            api,
            "enviar_dados_para_execucao_sp_sadt",
            Mock(side_effect=[{"message": "Dados incompletos", "status_code": 400}]),
        )
...
def test_o_rpa_nao_deve_ser_processado_devido_falha_na_construcao_xml_para_envio(
    mocker,
):
    ...
    with app.app_context():
        ...
        fake.construir_xml.side_effect = Exception(
            {"message": "Falha ao construir XML.", "status_code": 400}
        )
        ...
        mocker.patch.object(
            ...
            "enviar_dados_para_execucao_sp_sadt",
            Mock(
                side_effect=[{"message": "Falha ao construir XML.", "status_code": 400}]
            ),
        )
...
def test_o_processamento_do_rpa_deve_ser_interrompido_devido_ao_web_service_do_cliente_estar_indisponivel(
    mocker,
):
    ...
    with app.app_context():
        fake = Mock()
        fake.processar_rpa.side_effect = Exception(
            {"message": "Web service do cliente indisponível.", "status_code": 503}
        )
