...
@patch("requests.post")
def test_o_rpa_deve_ser_processado_com_o_envio_dos_dados_para_autorizacao(
    mock_request_post, caplog
):
    ...
    logging.info(
        f"XML construído para envio na API SOAP : {api.factory_request_sp_sadt_data_xml.xml}"
    )

    assert api.execucao_sp_sadt.response == "<xml>API SOAP Autorizado</xml>"
    logging.info(
        f"Resposta da solicitação de autorização no Web Service SOAP : {api.execucao_sp_sadt.response}"
    )

    assert api.execucao_sp_sadt.processado is True
    logging.info(
        f"O RPA foi exeutado com sucesso na API SOAP : {api.execucao_sp_sadt.processado}"
    )
