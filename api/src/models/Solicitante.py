    ...
    def buscar(where):
        try:
            ...
            if where.get("codigo_solicitante"):
                query = query.where(
                    Solicitante.codigo_solicitante == where["codigo_solicitante"]
                )

            if where.get("profissional_solicitante"):
                query = query.where(
                    Solicitante.profissional_solicitante
                    == where["profissional_solicitante"]
                )

            if where.get("conselho_profissional"):
                query = query.where(
                    Solicitante.conselho_profissional == where["conselho_profissional"]
                )

            if where.get("numero_conselho_profissional"):
                query = query.where(
                    Solicitante.numero_conselho_profissional
                    == where["numero_conselho_profissional"]
                )
