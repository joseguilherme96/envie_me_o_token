...
class Contratado(db.Model):
    ...
    carater_atendimento : Mapped[str] = mapped_column(Enum("1","2"),comment="1 - Eletiva, 2 - Urgência/Emergencia")
    tipo_atendimento : Mapped[str] = mapped_column(Enum(*valores,name="tipo_atendimento_enum",validate_strings=False),comment="Ver tiss/schemas.")
    ...
    def buscar(where):
        ...
        try:
            ...
            if where.get("codigo_prestador_na_operadora"):
                ...
                query = query.where(Contratado.codigo_prestador_na_operadora == where["codigo_prestador_na_operadora"])

            if where.get("nome_contratado"):

                query = query.where(Contratado.nome_contratado == where["nome_contratado"])

            if where.get("carater_atendimento"):

                query = query.where(Contratado.carater_atendimento == where["carater_atendimento"])

            if where.get("tipo_atendimento"):

                query = query.where(Contratado.tipo_atendimento == where["tipo_atendimento"])
