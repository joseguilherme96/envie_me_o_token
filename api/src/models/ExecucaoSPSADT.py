...
class ExecucaoSPSADT(db.Model):
    ...
    codigo_execucao : Mapped[int] = mapped_column(primary_key=True,unique=True,autoincrement=True)
    ...
    def buscar(where):
        try:
            ...
            if where.get("codigo_execucao"):
                query = query.where(ExecucaoSPSADT.codigo_execucao == where["codigo_execucao"])
            
            if where.get("codigo_beneficiario"):
                query = query.where(ExecucaoSPSADT.codigo_beneficiario == where["codigo_beneficiario"])
            
            if where.get("codigo_contratado"):
                query = query.where(ExecucaoSPSADT.codigo_contratado == where["codigo_contratado"])
            
            if where.get("codigo_solicitante"):
                query = query.where(ExecucaoSPSADT.codigo_solicitante == where["codigo_solicitante"])
            
            if where.get("operadora_registro_ans"):
                query = query.where(ExecucaoSPSADT.operadora_registro_ans == where["operadora_registro_ans"])
