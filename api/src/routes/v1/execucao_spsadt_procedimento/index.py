...
def cadastrar_execucao_spsadt_procedimento():
    try:
        ...
        notEmptyOrError(
            dados, "codigo_execucao", "O campo codigo_execucao deve ser preenchido !"
        )
        notEmptyOrError(
            dados,
            "descricao_procedimento",
            "O campo descricao_procedimento deve ser preenchido !",
        )
        notEmptyOrError(
            dados,
        ...
        if not isinstance(dados["descricao_procedimento"], str):
            return jsonify(
                {"message": "O campo descricao_procedimento deve ser um texto !"}
            ), 400
        ...
        execucao_sp_sadt = ExecucaoSPSADT.buscar(
        ...
        if not execucao_sp_sadt:
            return jsonify(
                {"message": "Não foi encontrado o codigo_execucao informado !"}
            ), 404

        dados = ExecucaoSPSADTProcedimento.inserir(
            ExecucaoSPSADTProcedimento(
