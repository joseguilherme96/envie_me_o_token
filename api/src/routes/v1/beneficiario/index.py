...
def cadastrar_beneficiario():
    try:
        ...
        try:
            ...
            return jsonify({"message": "O campo numero_carteira deve ser um inteiro !"}), 400

        if len(str(dados["numero_carteira"])) < 8 or len(str(dados["numero_carteira"])) > 12:
            return jsonify({"message": "O campo numero_carteira é inválido !"}), 400

        if not isinstance(dados["atendimento_rn"], bool):
            return jsonify({"message": "O campo atendimento_rn deve ser um booleano !"}), 400   
