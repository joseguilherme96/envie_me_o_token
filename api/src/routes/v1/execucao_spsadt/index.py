...
def cadastrar_execucao_spsadt():
    try:
        ...
        notEmptyOrError(dados, "codigo_beneficiario", "O campo codigo_beneficiario é obrigatório !")
        notEmptyOrError(dados, "codigo_contratado", "O campo codigo_contratado é obrigatório !")
        notEmptyOrError(dados, "codigo_solicitante", "O campo codigo_solicitante é obrigatório !")
        notEmptyOrError(dados, "operadora_registro_ans", "O campo operadora_registro_ans é obrigatório !")
        notEmptyOrError(dados, "login", "O campo login é obrigatório !")
        notEmptyOrError(dados, "indicacao_clinica", "O campo indicacao_clinica é obrigatório !")
        notEmptyOrError(dados, "indicacao_acidente", "O campo indicacao_acidente é obrigatório !")
        notEmptyOrError(dados, "observacao", "O campo observacao é obrigatório !")
        notEmptyOrError(dados, "senha", "O campo senha é obrigatório !")
        notEmptyOrError(dados, "tipo_transacao", "O campo tipo_transacao é obrigatório !")

        if isinstance(dados.get("codigo_beneficiario"), str) == False:
            return jsonify({"message": "O campo codigo_beneficiario deve ser um texto !"}), 400
        ...
        if isinstance(dados.get("tipo_transacao"), str) == False:
            return jsonify({"message": "O campo tipo_transacao deve ser um texto !"}), 400
        
        if not isinstance(dados.get("indicacao_clinica"), bool):
            return jsonify({"message": "O campo indicacao_clinica deve ser um booleano !"}), 400

        isNumericOrError(dados, "codigo_contratado", "O campo codigo_contratado deve ser um número inteiro !")
        isNumericOrError(dados, "codigo_solicitante", "O campo codigo_solicitante deve ser um número inteiro !")
        isNumericOrError(dados, "operadora_registro_ans", "O campo operadora_registro_ans deve ser um número inteiro !")
        isNumericOrError(dados, "indicacao_acidente", "O campo indicacao_acidente deve ser um número inteiro !")

        beneficiario = Beneficiario.buscar(where={"numero_carteira": dados["codigo_beneficiario"]})
        logging.debug(beneficiario)

        if not beneficiario:
            return jsonify({"message": "O codigo_beneficiario não está cadastrado !"}), 404
        
        contratado = Contratado.buscar(where={"codigo_prestador_na_operadora": dados["codigo_contratado"]})
        logging.debug(contratado)

        if not contratado:
            return jsonify({"message": "O codigo_contratado não está cadastrado !"}), 404
        
        solicitante = Solicitante.buscar(where={"codigo_solicitante": dados["codigo_solicitante"]})
        logging.debug(solicitante)

        if not solicitante:
            return jsonify({"message": "O codigo_solicitante não está cadastrado !"}), 404
        
        operadora = Operadora.buscar(where={"registro_ans": dados["operadora_registro_ans"]})
        logging.debug(operadora)
        
        if not operadora:
            return jsonify({"message": "O operadora_registro_ans não está cadastrado !"}), 404
        
        dados = ExecucaoSPSADT.inserir(ExecucaoSPSADT(
